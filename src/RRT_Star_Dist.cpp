...
public:
			...
			float d, float mu, Polygon_3 &Environment,Node_RRT_Pos x_rand[Node], bool Verbose)
	{
		...
		short int X_nearn[SIZE_XNEAR]={0};
		// list<int> X_near;
		...
		for(int i=1;i<Node;i++)
		{
			...
			Xil_DCacheFlushRange((UINTPTR)RRT_Starnb,(sizeof(Node_RRT_nb  )*i));
			Xil_DCacheFlushRange((UINTPTR)X_nearn,(sizeof( short int )*SIZE_XNEAR));

			XNearest_Set_N(&nearest,i);
			//XNearest_Set_xn(&nearest,i);
			XNearest_Set_x_rand(&nearest,*(u64*)(&x_rand[i]));
			...
			int status_tf=XAxiDma_SimpleTransfer(&axiDma,(UINTPTR)RRT_Starnb,
					(sizeof(Node_RRT_nb )*i),XAXIDMA_DMA_TO_DEVICE);
			...
			status_tf=XAxiDma_SimpleTransfer(&axiDma,(UINTPTR)X_nearn,
					(sizeof(short int)*SIZE_XNEAR),XAXIDMA_DEVICE_TO_DMA);
			if(status_tf!=XST_SUCCESS)
			{
			...
			XNearest_Read_oup_Words(&nearest,0,(long unsigned int *)(xpos_hw),3);
			//XNearest_Read_oup_Words(&nearest,0,(long unsigned int *)(rtdnv),1);
			int zz= XNearest_Get_return(&nearest);

			cout<<xpos_hw[2]<<","<<zz<<endl;

			if( Obstacle_Free( RRT_Star_Pos[x_nearest].Position, x_new_pos.Position,
					Squared_R_Robot, Environment ) )
			{
				...
				Near( X_near, RRT_Star_Pos, x_new_pos, Squared_r_n, i,X_near_count );
				...
				//XNearest_Set_xn(&nearest,i);
				XNearest_Set_x_rand(&nearest,*(u64*)(&x_rand[i]));
				XNearest_Set_choose(&nearest,2);
				XNearest_Set_Squared_r_n(&nearest,*(int*)(&Squared_r_n));
				...
				while(!XNearest_IsDone(&nearest));
				Xil_DCacheInvalidateRange((u32)X_nearn,(sizeof(short int)*SIZE_XNEAR));
				//XNearest_Read_oup_Words(&nearest,0,(long unsigned int *)(xpos_hw),3);
				//XNearest_Read_oup_Words(&nearest,0,(long unsigned int *)(rtdnv),1);
				int zz= XNearest_Get_return(&nearest);

				cout<<X_near_count<<","<<zz<<endl;


				XTime_GetTime(&sw_processor_end);
				float sw_processing_time=1000000.0*(sw_processor_end-sw_processor_start)/(COUNTS_PER_SECOND);
				//cout <<sw_processing_time << endl;


				x_min = x_nearest;
				//cout<<"xmin "<<x_min<<" i "<<i<<endl;
				c_min_local = Cost( RRT_Star_Pos[x_nearest ].Position, x_new_pos.Position );
				...
				for(int xnear_index=0;xnear_index<X_near_count;xnear_index++)
				{
					...
					cost_tem_local = Cost( RRT_Star_Pos[pos_x_near ].Position, x_new_pos.Position );
					cost_tem = RRT_Star[pos_x_near ].Cost + cost_tem_local;

					if( Obstacle_Free( RRT_Star_Pos[pos_x_near ].Position, x_new_pos.Position,
							Squared_R_Robot, Environment))
					{
				...
				//cout<<i<<endl;
				//cout<<RRT_Star_Pos[i].Position.x<<endl;
				...
				for (int list_pos_ele_ind=0;list_pos_ele_ind<List_pos_near_index;list_pos_ele_ind++)
				{
					...
					cost_tem_local = Cost( RRT_Star_Pos[i].Position,
							RRT_Star_Pos[tem_pos_x_near ].Position );
					...
					if( cost_tem < RRT_Star[tem_pos_x_near ].Cost )
					{
						...
						ARewire_the_tree( tem_pos_x_near, RRT_Star ,RRT_Star_Pos);
					}
	...
	static void ARewire_the_tree( int Pos_x, Node_RRT RRT_Star[Node] , Node_RRT_Pos RRT_Star_Pos[Node] )
	{
		int list_n[30];
		short count_list=0;
		list_n[0]=Pos_x;
