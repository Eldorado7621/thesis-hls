...
class Auxiliary
{
    public:
		...
		static int  Read_xrand(Node_RRT_Pos x_rand[Node]){
			int Status;
...
/*    		for (auto g : y)
    	...
    	/*static void Save_Trajectory( Node_RRT G[Node], int i, string Name )
    	{
    		...
    		fptry = openFile("trajy.txt",'w');
    		fptrx = openFile("trajx.txt",'w');
