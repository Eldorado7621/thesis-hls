...
/*	float test_data[10]={1.02,2.02,3.09,4.09,3.04,5.09,5.90,6.90,8.98,9.89};
    ...
    Polygon_2 Env; //Environment
    Polygon_3 Environment; //Environment
    Polygon_4 Envr;
    Node_RRT RRT_Star[Node];
    Node_RRT_Pos RRT_Star_Pos[Node];
    Node_RRT_Pos x_rand[Node];
    ...
    Auxiliary::Read_data( Pos_Init, Pos_Goal, Total_Num_Node, R_Robot, Radio_Goal,
	    Gamma_rrt, Mu, Env, Name );
    Total_Num_Node=Node;
    Region RRT_Star_Region( Env, R_Robot );
    int envi=0;


      for(auto it = Env.vertex.begin(); it != Env.vertex.end(); ++it) {
          Environment.vertices[envi]=*it;
          envi++;
      }
      for (int i=0; i<4;i++)
      {
    	  Envr.edges[i].Source=Env.edge[i].source();
    	  Envr.edges[i].Target=Env.edge[i].target();
      }
...
/*    for (int i = 0; i < 5; i++)
    ...
    RRT_Star_Dist::Build_RRT_Star( RRT_Star, RRT_Star_Pos,Pos_Init, Total_Num_Node, R_Robot, Gamma_rrt,
