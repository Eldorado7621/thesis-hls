...
bool Colition( Point_2 &A, float Squared_R_Robot, Segment_3 Edges[MAX_EDGES])
{
	bool Bool = false;
...
static void Near(   int X_near[SIZE_NEAR], Node_RRT_Pos RRT_Star_Pos[Node], Node_RRT_Pos &x_new, float Squared_r_n, int N , int &X_near_count)
{
	X_near_count=0;
	for( int i=0; i<N; i++ )
	{
...
bool nearest(hls::stream<axis_data> &dataInStream,Node_Rand x_rand,int N,
		int choose,int oup[3],float Squared_r_n,
...
#pragma HLS PIPELINE
	...
	oup[2]=RRT_Star[N-1].Location_List;
